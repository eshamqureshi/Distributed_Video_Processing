...
def process_chunk():
    try:
        ...
        print(f"Worker 2: Received and saved chunk: {chunk_filename} for action: {action}")
        ...
        if action == 'encode':
            ...
            print(f"Worker 2: Encoded (inverted) chunk: {chunk_filename}")
        elif action == 'decode':
            # Reverse color inversion (since inversion is its own inverse)
            processed_video = video.fl_image(invert_colors)
            processed_chunk_filename = f"processed_{chunk_filename}"
            print(f"Worker 2: Decoded (re-inverted) chunk: {chunk_filename}")
        ...
        print(f"Worker 2: Saved processed chunk as {processed_chunk_filename}")
